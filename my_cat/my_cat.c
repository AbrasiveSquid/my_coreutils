...
void parse_arguments(int size, char * arg_list[], File_List * files, Options * flags);
...
int main(int argc, char *argv[]) 
{
  ...
  File_List file_list = {NULL, 0}; // init files array to NULL and file_count to 0
  ...
  for (size_t i = 0; i < file_list.file_count; i++) 
  {
    ...
    else 
    {
      ...
      if (fp == NULL) {
        fprintf(stderr, "%s: %s: No such file or directory", argv[0], *(file_list.files));
        continue;
      }
...
void parse_files(char * file_name, File_List * file_list) 
{
  ...
  else
  {
    ...
    char ** file_list_temp = realloc(file_list->files, (file_list->file_count + 1) * sizeof(file_list->files));
    if (*file_list_temp)
    {
      perror(file_name);
      cleanup(&(file_list->files));
...
{
  int i;
  int exit_status; // exit return code depending on valid or invalid flag
  ...
  for (i = 1; i < size; i++) 
  {
    ...
    if (argv[i][0] == '-' && argv[i][1] != '\0' ) 
    {
      ...
      if (flags->exit_early) // some flags or invalid option cause program to exit early
      {
        if (flags->show_help)
        {
          print_help(argv[0]);
